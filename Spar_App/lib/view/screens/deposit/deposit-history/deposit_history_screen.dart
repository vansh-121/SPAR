    ...
    return GetBuilder<DepositController>(
      builder: (controller) => SafeArea(
        child: Scaffold(
          ...
          appBar: AppBar(
            title: Text(MyStrings.depositHistory.tr,
            ...
            actions: [
              // Disabled deposit action in Plan History screen
              GestureDetector(
                onTap: () {
                ...
                child: Container(
                  ...
                  child: Icon(controller.isSearch ? Icons.clear : Icons.search,
                      color: MyColor.getSelectedIconColor(), size: 15),
                ),
              ),
            ],
          ),
                                      ...
                                      itemBuilder: (context, index) {
                                        ...
                                        return CustomDepositCard(
                                          ...
                                          gatewayData: controller
                                              ...
                                              "",
                                          conversionData:
                                              "${Converter.twoDecimalPlaceFixedWithoutRounding(controller.depositList[index].finalAmo ?? "")} ${controller.depositList[index].methodCurrency}",
                                          amountConversion:
                                              "1 ${controller.currency} = ${Converter.twoDecimalPlaceFixedWithoutRounding(controller.depositList[index].rate ?? "")} ${controller.depositList[index].methodCurrency}",
                                          amountData:
                                              "${double.parse(Converter.twoDecimalPlaceFixedWithoutRounding(controller.depositList[index].amount ?? "")) + double.parse(Converter.twoDecimalPlaceFixedWithoutRounding(controller.depositList[index].charge ?? ""))} ${controller.currency}",
                                          statusData:
