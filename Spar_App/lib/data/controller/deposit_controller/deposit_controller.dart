...
class DepositController extends GetxController {
  ...
  beforeInitLoadData() async {
    ...
    depositModel = await depositRepo.getDepositHistory(
      page: page,
    );

    depositList.clear();
    List<DepositHistoryListModel>? tempDepositList =
        depositModel.data?.deposits?.data;
    // Filter only plan-related deposits (have plan_id)
    if (tempDepositList != null) {
      tempDepositList = tempDepositList.where((e) {
        // plan_id exists in raw json but model lacks field; detect via presence of gateway and method_code >= 1000 or keep all
        // Here we keep all and rely on backend already scoping by plan; adjust if planId is added to model
        return true;
      }).toList();
    }
  ...
  void searchDepositTrx() async {
    ...
    depositModel =
        await depositRepo.getDepositHistory(isSearch: true, trx: trx);
    List<DepositHistoryListModel>? tempDepositList =
        depositModel.data?.deposits?.data;
    if (tempDepositList != null) {
      tempDepositList = tempDepositList.where((e) {
