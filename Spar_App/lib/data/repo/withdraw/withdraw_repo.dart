...
class WithdrawRepo {
  ...
  Future<ResponseModel> getUserWalletBalances() async {
  ...
  Future<dynamic> addWithdrawRequest(
      int methodCode, double amount, String wallet) async {
    ...
    developer.log('🌐 API CALL - WITHDRAW REQUEST', name: 'WithdrawRepo');
    developer.log('   • URL: $url', name: 'WithdrawRepo');
    developer.log('   • Parameters:', name: 'WithdrawRepo');
    developer.log('     - method_code: ${params['method_code']}',
        name: 'WithdrawRepo');
    developer.log('     - amount: ${params['amount']}', name: 'WithdrawRepo');
    developer.log('     - wallet: ${params['wallet']}', name: 'WithdrawRepo');

    ResponseModel responseModel = await apiClient
        .request(url, request.Method.postMethod, params, passHeader: true);

    developer.log('   • Response Status: ${responseModel.statusCode}',
        name: 'WithdrawRepo');
    developer.log('   • Response Message: ${responseModel.message}',
        name: 'WithdrawRepo');
    if (responseModel.statusCode == 200) {
      developer.log('   • Response Body: ${responseModel.responseJson}',
          name: 'WithdrawRepo');
    }
