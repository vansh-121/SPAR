...
class InvestController extends Controller
{
    public function invest(Request $request)
    {
        ...
        // Block multiple purchases of the same plan (web)
        $hasPlan = \App\Models\Invest::where('user_id', auth()->id())
            ->where('plan_id', $plan->id)
            ->whereIn('status', [Status::INVEST_RUNNING])
            ->exists();

        if ($hasPlan && !$request->invest_id) {
            $notify[] = ['error', 'You already own this plan. Please use Add Money to top up.'];
        ...
        // Enforce deposit-first flow for plan purchase (no direct wallet invest)
        if ($wallet === 'deposit_wallet' || $wallet === 'interest_wallet') {
            $notify[] = ['error', 'Please select a payment gateway'];
            return back()->withNotify($notify);
        }

        // Gateway checkout → create deposit tied to plan, admin approval will auto-invest
        ...
        $data = PaymentController::insertDeposit($gate, $request->amount, $plan, $compoundTimes);
