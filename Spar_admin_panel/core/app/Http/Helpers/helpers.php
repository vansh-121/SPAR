...
function notify($user, $templateName, $shortCodes = null, $sendVia = null, $createLog = true, $pushImage = null, $attachment = null) {
    $globalShortCodes = [
    ...
    if (is_array($shortCodes) && array_key_exists('cta_url', $shortCodes)) {
        $shortCodes['cta_url'] = normalizeCtaUrl($shortCodes['cta_url']);
    }
...
function normalizeCtaUrl($url) {
    ...
    if (substr($normalized, 0, 2) === '//') {
        $normalized = 'https:' . $normalized;
    } elseif (!preg_match('#^[a-z][a-z0-9+\-.]*://#i', $normalized)) {
