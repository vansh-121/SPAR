@forelse ($plans as $k => $data)
    <div class="@if(@$layout == 'master') col-xxl-3 col-sm-6 @else col-xl-3 col-sm-6 @endif">
        <div class="plan-card h-100">
            <span class="plan-card__badge"> {{ __($data->name) }} </span>
            <div class="plan-card__top">
                <span class="plan-card__number">
                    @if($data->interest_type == 1)
                        {{ showAmount($data->interest, currencyFormat:false) }}%
                    @else 
                        {{ gs('cur_sym') }}{{ showAmount($data->interest, currencyFormat:false) }}
                    @endif
                </span>
                <h6 class="plan-card__time">{{ __($data->timeSetting->name) }}</h6>
            </div>
            <ul class="plan-list">
                <li class="plan-list__item">
                    @lang('For') @if ($data->lifetime == 0)
                ...
                @if ($data->fixed_amount == 0)
                    <li class="plan-list__item">
                        @lang('Min') {{ gs('cur_sym') }}{{ showAmount($data->minimum, currencyFormat:false) }}
                    </li>
                    <li class="plan-list__item">
                        @lang('Max') {{ gs('cur_sym') }}{{ showAmount($data->maximum, currencyFormat:false) }}
                    </li>
                @else
                    <li class="plan-list__item">
                        @lang('Amount') {{ gs('cur_sym') }}{{ showAmount($data->fixed_amount, currencyFormat:false) }}
                    </li>
                @endif
            </ul>
            <div class="plan-card__btn">
                <button data-bs-toggle="modal" data-bs-target="#investModal" data-plan="{{ $data }}" class="btn--base btn w-100 investModal">@lang('Invest Now')</button>
            </div>
        </div>
    </div>
@empty
...
<div class="modal fade" id="investModal">
    <div class="modal-dialog @if(auth()->check()) modal-lg @endif modal-dialog-centered modal-content-bg">
        <div class="modal-content">
            ...
            <form action="{{ route('user.invest.submit') }}" method="post">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select class="form--control select2" name="wallet_type" required>
                                        <option value="">@lang('Select Payment Gateway')</option>
                            ...
                            <div class="col-md-6 compoundInterest">
                                <div class="form-group">
                                    <label class="form--label">@lang('Compound Interest') (@lang('optional'))</label>
                                    <div class="input-group">
                                        <input type="number" min="0" class="form--control form-control form-two" name="compound_interest">
                                        <div class="input-group-text input-style">@lang('Times')</div>
                                    </div>
                            ...
                            @if (gs('schedule_invest'))
                                <div class="col-md-6 investTime">
                                    <div class="form-group">
                                        <label class="form--label">@lang('Auto Schedule Invest')</label>
                                        <select class="form--control select2" name="invest_time" required data-minimum-results-for-search="-1">
                        ...
                        @if (gs('schedule_invest'))
                            <div class="row schedule">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label class="required form--label">@lang('Schedule For')</label>
                                        <div class="input-group">
                                            <input type="number" min="0" class="form--control form-control form-two" name="schedule_times">
                                            <span class="input-group-text  input-style">@lang('Times')</span>
                                        </div>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label class="required form--label">@lang('After')</label>
                                        <div class="input-group">
                                            <input type="number" class="form--control form-control form-two" name="hours" min="0">
                                            <span class="input-group-text input-style">@lang('Hours')</span>
                                        </div>
                ...
                <div class="modal-footer">
                    ...
                    @else
                        <a href="{{ route('user.login') }}" class="btn btn--base w-100 text-center">@lang('At first sign in your account')</a>
                    @endif
                </div>
            </form>
        </div>
